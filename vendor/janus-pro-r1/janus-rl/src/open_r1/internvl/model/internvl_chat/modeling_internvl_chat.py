...
class InternVLChatModel(PreTrainedModel):
    ...
    def my_batch_chat2(self, tokenizer, pixel_values, questions, generation_config, num_patches_list=None,
        ...
        for idx in range(len(num_patches_list)//2):
            ...
            image_tokens = IMG_START_TOKEN + IMG_CONTEXT_TOKEN * self.num_image_token * num_patches_1 + IMG_END_TOKEN
            query = query.replace('<image>', image_tokens, 1)
            image_tokens = IMG_START_TOKEN + IMG_CONTEXT_TOKEN * self.num_image_token * num_patches_2 + IMG_END_TOKEN
            query = query.replace('<image>', image_tokens, 1)
            queries.append(query)
