...
class GRPOTrainer(Trainer):
    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        completion_mask_3 = torch.ones((img_ids_2.size(0), img_ids_2.size(1)), dtype=torch.long, device=device)
        ...
        logits_to_keep_3 = img_ids_2.size(1)
        ...
        with torch.inference_mode():
            ...
            if self.ref_model is not None:
                ref_per_token_logps_3 = self._get_per_token_logps(
                    self.ref_model, input_embeds=embeds_3, output_ids=img_ids_2, attention_mask=attention_mask_3, logits_to_keep=logits_to_keep_3, addcfg=True, visual=True
                )
            else:
                raise NotImplementedError
        ...
        self._metrics["rewards/selfcheck"].append(all_self_check)
        ...
        names = ['first_gen', 'compre', 'second_gen']

        final_rewards = torch.zeros(len(prompts), device=device, dtype=rewards1.dtype)
        for i in range(len(selfcheck)):
          if selfcheck[i]==True:
            final_rewards[i] = rewards1[i]
          else:
            final_rewards[i] = rewards3[i]
        self._metrics["finalgen"].append(self.accelerator.gather_for_metrics(final_rewards).mean().item())
