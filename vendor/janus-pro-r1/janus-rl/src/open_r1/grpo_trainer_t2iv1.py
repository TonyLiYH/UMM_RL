    ...
    def _prepare_inputs(self, inputs: dict[str, Union[torch.Tensor, Any]]) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        if self.args.use_vllm:
            ...
            with unwrap_model_for_generation(self.model, self.accelerator, gather_deepspeed3_params=False) as unwrapped_model:
                ...
                with torch.no_grad():
                    padding_len = embeds_3.size(1) - embeds_1.size(1)
                    padding_mask = torch.zeros((attention_mask_1.size(0), padding_len), dtype=attention_mask_1.dtype, device=attention_mask_1.device)
                    pad_input_ids = torch.ones((embeds_1.size(0), padding_len), dtype=prompt_ids.dtype, device=embeds_1.device) * self.vl_chat_processor.pad_id
                ...
                for i in range(len(newselfcheck)):
                    if newselfcheck[i]==True:
                        ...
                        gen_attention_mask[i, :] = attention_mask_1_pad[i, :]
                    else:
                ...
                for ind in range(selfcheck.size(0)):
                    if selfcheck[ind]==True:
                        imgs_2.append(all_imgs_1[ind])
                        img_ids[ind, :] = img_ids_1[ind, :]
                    else:
        ...
        return [
            {
                ...
                "prompt_embeds": gen_embeds,
                "prompt_mask": gen_attention_mask,
                "completion_ids": img_ids,
                'completion_mask': gen_completion_mask,
